export const NumeroALetras = (function() {
    ...
    return function NumeroALetras(num:number, currency:any) {
        ...
        const data = {
            ...
            letrasCentavos: '',
            letrasMonedaPlural: currency.plural || 'SOLES', //'PESOS', 'Dólares', 'Bolíconstes', 'etcs'
            letrasMonedaSingular: currency.singular || 'SOL', //'PESO', 'Dólar', 'Boliconst', 'etc'
        ...
        if (data.centavos > 0) {
            data.letrasCentavos = 'CON ' + (function() {
                if (data.centavos == 1)
                    return Millones(data.centavos) + '/100' + 'SOLES';
                else
                    return Millones(data.centavos) + '/100' + 'SOLES';
            })();
        };
        ...
        if (data.enteros == 1)
            return Millones(data.enteros) + ' CON 0/100 ' + data.letrasMonedaSingular + ' ' + data.letrasCentavos;
        else
            return Millones(data.enteros) + ' CON 0/100 ' + data.letrasMonedaPlural + ' ' + data.letrasCentavos;
    };
